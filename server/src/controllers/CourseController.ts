...
      const coursesWithInstructorName = data?.map(course => {
        const { profiles, ...restOfCourse } = course;
        return {
          ...restOfCourse,
...
      if (!courseId) {
        res.status(400).json({ data: null, error: { message: 'ID del curso es requerido' } });
        return;
      }

      const { data, error } = await supabase
        .from('courses')
        .select('*, modules(*, lessons(*), evaluations(*)), profiles(full_name)')
        .eq('id', courseId)
        .single();
      
      if (error) throw error;
      
      const { profiles, ...restOfData } = data;
