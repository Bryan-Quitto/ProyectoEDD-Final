    ...
    switch(action.target) {
        ...
        case 'FIRST_ADVANCED_OR_CORE_LESSON': {
            ...
            if (lesson) {
                ...
                recommendationData.action_url = `/courses/${courseId}/modules/${moduleId}/lessons/${lesson.id}`;
            } else {
  ...
  private async findFirstLessonByLevel(moduleId: string, level: Lesson['target_level']): Promise<ApiResponse<Lesson | null>> {
    try {
        const { data, error } = await supabase
            ...
            .limit(1)
            .maybeSingle(); // Usamos maybeSingle para que no dé error si no encuentra filas
