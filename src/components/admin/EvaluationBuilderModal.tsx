...
interface EvaluationBuilderModalProps {
  ...
  existingEvaluation?: Partial<Evaluation>;
}

export const EvaluationBuilderModal: React.FC<EvaluationBuilderModalProps> = ({ isOpen, onClose, onSave, existingEvaluation }) => {
  ...
  useEffect(() => {
    if (isOpen) {
        const defaultQuestions = [{ id: crypto.randomUUID(), question_text: '', question_type: 'multiple_choice' as const, points: 10, options: ['', ''], correct_options: [] }];
        reset(existingEvaluation && existingEvaluation.questions && existingEvaluation.questions.length > 0 ? existingEvaluation : { ...existingEvaluation, questions: defaultQuestions, evaluation_type: 'quiz' });
    }
  }, [isOpen, existingEvaluation, reset]);
