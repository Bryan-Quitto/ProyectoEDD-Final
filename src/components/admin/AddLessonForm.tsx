...
    defaultValues: { title: '', lesson_type: 'text', target_level: 'core', estimated_duration: 5, content: '' }
  });
...
              <Controller name="lesson_type" control={control} render={({ field }) => ( <select {...field} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"> <option value="text">Texto Plano</option> <option value="fill_in_the_blank">Completar Espacios</option> <option value="quiz">Autoevaluación (Quiz)</option> </select> )} />
            </div>
            <div>
...
                {evaluationData ? 'Editar Autoevaluación' : 'Diseñar Autoevaluación'}
              </Button>
            </div>
          )}
          {lessonType === 'text' && (
